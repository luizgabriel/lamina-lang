use crate::{
    ir::{IrExpr, IrExprNode, lowering_expr, lowering_stmt},
    lexer::Spanned,
...
impl Compiler {
    ...
    fn compile_ir(&mut self, expr: &IrExpr<'_>) {
        match expr.as_ref() {
            IrExprNode::Ident(name) => {
                self.instructions
                    .push(Instruction::LoadVar(name.to_string()));
            }
            IrExprNode::Literal(literal) => {
                self.instructions
                    .push(Instruction::LoadConst(literal.clone().into()));
            }
            IrExprNode::Tuple(items) => {
                let size = items.len();
                for item in items {
            ...
            IrExprNode::Lambda { arg, body } => {
                let body_instructions = self.compile_function_body(&body.0);
